...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_save_users_to_preferences.setOnClickListener {
            ...
            Toast.makeText(this,"Saved ${usersToSave.size} users to precerences.", Toast.LENGTH_SHORT).show()
        }
    }

    private fun saveUsersToPreferences(users: List<User>){
        val prefEditor = PreferenceManager.getDefaultSharedPreferences(this).edit()
        val jsonString = Gson().toJson(users) //para convertir la lista de usuarios
        prefEditor.putString("users",jsonString).apply() //apply para aplicar las preferencias al editor
    }
